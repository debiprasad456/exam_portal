...
export default function AdminLogin() {
  ...
  useEffect(() => {
    ...
    checkAdminExists()
      .then(({ data }) => {
        if (!data.exists) setMode('setup');
      })
      .catch((err) => {
        if (!err.response) {
          setError(
            'Backend server is waking up or unreachable. Please wait 10 seconds and refresh the page.'
          );
        }
      })
  ...
  // Standard Login & First-time Setup
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'setup') {
        ...
        setEmail('');
        setPassword('');
        setSuccessMsg('Admin account created successfully! Please sign in.');
      } else {
      ...
      if (!err.response) {
        setError(
          'Cannot connect to backend server. Render free backend may be starting up—please wait 10–20 seconds and click Sign In again.'
        );
      } else {
  ...
  const handleResetPasswordSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to reset password. Please verify the code and try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen page-bg flex items-center justify-center p-4">
      <div className="w-full max-w-md animate-slide-up">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2">
            {mode === 'setup'
              ...
              : mode === 'forgot'
              ? 'Recover access to your administrator account'
        ...
        <div className="glass-card p-5 sm:p-8">
          {mode !== 'forgot' && <AuthRoleToggle activeRole="admin" />}
          ...
          {mode === 'forgot' && (
            <p className="text-xs text-slate-400 mb-6">
              {forgotStep === 1
                ...
                : `Verification code sent to ${email}. Enter the code and your new password.`}
            </p>
          )}

          {error && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-red-500 bg-opacity-10 border border-red-500 border-opacity-30 text-red-400 text-sm animate-fade-in">
              {error}
            </div>
          )}

          {successMsg && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-emerald-500 bg-opacity-10 border border-emerald-500 border-opacity-30 text-emerald-400 text-sm animate-fade-in">
              {successMsg}
            </div>
          )}

          {/* Mode: Standard Login or Setup */}
          {mode !== 'forgot' && (
                ...
                <form onSubmit={handleResetPasswordSubmit} className="flex flex-col gap-4">
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="form-label mb-0">6-Digit Verification Code</label>
                      <button
                        type="button"
                    ...
                    <input
                      ...
                      placeholder="123456"
                      className="form-input text-center text-xl font-mono tracking-widest"
                      required
                      autoFocus
                    />
                  </div>
